import { SessionRepositoryPort } from "@clininote/api/application/ports/session-repository.port.js";
import { SessionModel } from "@clininote/api/domain/entities/session.js";
...
export class InMemorySessionRepository implements SessionRepositoryPort {
    ...
    async update(session: SessionModel): Promise<SessionModel> {
        if (!this.sessions.has(session.id)) {
            throw new Error(`Clinical note with id ${session.id} was not found.`);
        }
